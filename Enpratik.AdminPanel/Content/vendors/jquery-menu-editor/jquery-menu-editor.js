...
function MenuEditor(idSelector, options) {
    ...
    function createMenu(arrayItem, depth) {
        ...
        $.each(arrayItem, function (k, v) {
            ...
            var itemObject = { menuName: "", menuPage: "", target: "_self", href: "" };
            var temp = $.extend({}, v);
            if (isParent){ 
            ...
            var $span = $('<span>').addClass('txt').append(v.menuName).css('margin-right', '5px');
    ...
    this.update = function(){
        ...
        $form.find('.item-menu').each(function(){
            $cEl.data($(this).attr('menuName'), $(this).val());
        });
        $cEl.children().children('i').removeClass(oldIcon).addClass($cEl.data('icon'));
        $cEl.find('span.txt').first().text($cEl.data('menuName'));
        resetForm();
    };
    ...
    this.add = function(){
        var data = {};
        $form.find('.item-menu').each(function(){
            data[$(this).attr('menuName')] = $(this).val();
        });
        var btnGroup = TButtonGroup();
        var textItem = $('<span>').addClass('txt').text(data.menuName);
