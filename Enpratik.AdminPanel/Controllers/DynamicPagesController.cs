...
namespace Enpratik.AdminPanel.Controllers
{
    public class DynamicPagesController : BaseController
    {
        ...
        {
            dynamicPages.PageContent = string.Empty;
            if (ModelState.IsValid)
            {
        ...
        public ActionResult Edit(DynamicPages dynamicPages)
        {
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    dynamicPages.PageContent = dynamicPages.PageContent.Replace("../../../Themes", "/Themes");
                    dynamicPages.PageContent = dynamicPages.PageContent.Replace("../", "").Replace("Upload", "/admin/Upload").Replace("//admin", "/admin").Replace("/admin/admin/", "/admin/");
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult TemplateEdit(int? id, string pageContent)
        {
            DynamicPages dynamicPages = new DynamicPages();
            if (ModelState.IsValid)
            {
                ...
                {
                    dynamicPages = db.DynamicPages.Find(id);
                    dynamicPages.PageContent = pageContent;
                    dynamicPages.UpdatedDate = DateTime.Now;
                    dynamicPages.UpdatedUserId = 1;
                    dynamicPages.IsActive = true;
                    db.Entry(dynamicPages).State = EntityState.Modified;
                    db.SaveChanges();
                    Functions.SetMessageViewBag(this, "Kayıt başarıyla güncellendi!", 1);
                }
            }
            return View(dynamicPages);
        }
